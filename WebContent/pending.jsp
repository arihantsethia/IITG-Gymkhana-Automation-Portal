...
<body>
	...
	<font color="gray" size="3">
		<center>
			<p>
				<strong>Following requests are pending :</strong>
			</p>
			<table id="dynamic" cellpadding="1" border="1" cellspacing="1">
				<tr>
					...
					<th width=10%>Approval Official</th>
					<th width=4%>Room Alloted</th>
				</tr>

				<%
					System.out.println(session.getAttribute("id"));
					String sql = "SELECT * FROM request WHERE origin='"
							+ session.getAttribute("user") + "' OR approvedBy='"
							+ session.getAttribute("user")
							+ "' AND status != 1 AND bRoom = 'No'";
					Connection con = null;
					Statement s = null;
					ResultSet rs = null;
					try {
				...
				<tr>
					...
					<%
						if (session.getAttribute("user").toString().equals("dosa")) {
					%>
					<td>Room Allocation Pending</td>
					<%
						} else {
					%>
