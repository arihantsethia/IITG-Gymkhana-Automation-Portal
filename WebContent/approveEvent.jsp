...
<body>
	<%
		...
		if (name == null || Integer.valueOf(session.getAttribute("level").toString())==0) {
			...
			<table id="dynamic" cellpadding="1" border="1" cellspacing="1">
				...
				<%
					...
					String sql = "SELECT * FROM request WHERE approvalOf = '"
							...
							+ "' AND approvedBY != '"+session.getAttribute("user")+"'AND status = 0";
					Connection con = null;
					Statement s = null;
					ResultSet rs = null;
					try {
				...
				<tr>
					...
					<input type="hidden" name="approved" value="<%=rs.getString(13)%>" />
					<input type="hidden" name="done" value="<%=rs.getString(14)%>" />
					<input type="hidden" name="approvedBy" value="<%=session.getAttribute("user") %>" />					
					<input type="hidden" name="bRoom" value="<%=rs.getString(11)%>" />
					<input type="hidden" name="approvalOf" value="<%=session.getAttribute("super") %>" />	
